...
class Nominal
{
    ...
    private const array backWovels = ["a", "o", "u"];
    private const array frontWovels = ["y", "ä", "ö"];
    ...
    */
    private function isBackWovelWord(string $word): ?bool
    {
      // if the same word is being checked, use the previous result instead of matching again
        ...
        $backVowelPos = -1;
        foreach (self::backWovels as $wovel) {
            $pos = mb_strrpos($word, $wovel);
        ...
        $frontVowelPos = -1;
        foreach (self::frontWovels as $wovel) {
            $pos = mb_strrpos($word, $wovel);
